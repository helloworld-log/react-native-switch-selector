...
export default class SwitchSelector extends Component {
  ...
  render() {
    ...
    const options = this.props.options.map((element, index) => {
      ...
      const isToday = element.value === moment().format('YYYY-MM-DD')
        ...
        >
          ...
          <Text
            style={[
              {
                fontSize,
                fontWeight: isToday ? '500' : bold ? "bold" : "normal",
                textAlign: "center",
                color: is_selected ? selectedColor : isToday ? 'rgb(101,195,239)' : textColor,
